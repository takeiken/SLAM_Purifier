...
class SLAMProcessor:
    ...
    def analyze_video(self, path, v_idx, v_total):
        ...
        blur_scores, shift_scores, prev_gray = [], [], None
        ...
        return self.get_intervals(blur_scores, shift_scores, fps), proc_time
    ...
    def process_all(self, on_finish):
        ...
        for i, filename in enumerate(files):
            ...
            intervals, proc_time = self.analyze_video(path, i, len(files))
            
            # --- "UNCLEAR" LOGIC ---
            if not intervals:
                interval_data = "Unclear"
                self.log_fn(f"⚠️ {filename}: No clear footage found.")
            else:
                interval_data = str(intervals)
                if not self.stop_event.is_set():
                    self.export_clips(path, filename, intervals)
            
            records.append({"Filename": filename, "Intervals": interval_data, "ProcTime_Sec": proc_time})
            self.save_csv(records)

        total_duration = round(time.time() - batch_start, 2)
        avg_time = round(total_duration / len(files), 2) if files else 0
        
        self.log_fn(f"--- BATCH EFFICIENCY ---")
        self.log_fn(f"Total Videos: {len(files)} | Total Time: {total_duration}s | Avg: {avg_time}s")
        
        with open(self.csv_path, 'a', newline='') as f:
            f.write(f"\nTOTAL BATCH TIME,{total_duration},AVERAGE PER VIDEO,{avg_time}\n")
            
        on_finish()

    def save_csv(self, records):
        with open(self.csv_path, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=["Filename", "Intervals", "ProcTime_Sec"])
...
class SLAMGui(ctk.CTk):
    def __init__(self):
        super().__init__()
        # FIX: Theme Sync forced to Dark at initialization
        ctk.set_appearance_mode("dark")
        self.title("SLAM Purifier v5.0")
        ...
        self.instr = ctk.CTkTextbox(self.sidebar, width=260, height=220, fg_color="#000000", text_color="#FFFFFF", font=("Arial", 12))
        self.instr.pack(padx=15, pady=5)
        step_text = (
            "1. SELECT FOLDER: Choose the directory containing your raw .mp4 or .mov files.\n\n"
            "2. BATCH NAME: Enter a tag (e.g., Reef_Site_A) to label your output files.\n\n"
            "3. THRESHOLDS: Set Sharpness (min) and Stability (max). Use Presets for quick setup.\n\n"
            "4. START: Click to run. UI will lock until batch finishes or STOP is clicked.\n\n"
            "5. OUTPUT: Check the CSV in the new timestamped folder for processing times."
        )
        ...
        self.mode_switch.select(); self.mode_switch.pack(pady=15)

        ctk.CTkLabel(self.sidebar, text="Batch Name Tag:", font=("Arial", 12, "bold")).pack(pady=(10,0))
        self.batch_entry = ctk.CTkEntry(self.sidebar, placeholder_text="e.g. Dive_Session_1", width=250)
        self.batch_entry.pack(pady=5)

        ctk.CTkLabel(self.sidebar, text="Underwater Presets:").pack(pady=(20, 0))
        self.preset_menu = ctk.CTkOptionMenu(self.sidebar, values=["Manual", "Reef Survey", "Macro", "Murky Water"], command=self.apply_preset)
        ...
        self.preview_label = ctk.CTkLabel(self.main_frame, text="Video Preview", fg_color="black", width=640, height=360, corner_radius=8)
        self.preview_label.pack(pady=10)

        # LIVE SCOPE
        self.scope_frame = ctk.CTkFrame(self.main_frame, fg_color="#1a1a1a", corner_radius=10)
        self.scope_frame.pack(pady=5, padx=10, fill="x")
        scope_inner = ctk.CTkFrame(self.scope_frame, fg_color="transparent"); scope_inner.pack(pady=10, padx=20)
        
        self.blur_scope_label = ctk.CTkLabel(scope_inner, text="SHARPNESS: 0 / 70", font=("Arial", 12, "bold"), text_color="white")
        self.blur_scope_label.grid(row=0, column=0, padx=25)
        ...
        self.shift_scope_label = ctk.CTkLabel(scope_inner, text="STABILITY: 0.0 / 4.0", font=("Arial", 12, "bold"), text_color="white")
        self.shift_scope_label.grid(row=0, column=1, padx=25)
        ...
        self.frame_label = ctk.CTkLabel(self.scope_frame, text="Frame: 0 / 0", font=("Courier", 12, "bold"))
        self.frame_label.pack(pady=(0, 5))

        # PATH DISPLAY
        self.path_label = ctk.CTkLabel(self.main_frame, text="Selected Folder: None", font=("Arial", 11), text_color="#3498db")
        ...
        self.btn_run = ctk.CTkButton(btn_frame, text="START PROCESS", fg_color="#2ecc71", width=200, height=40, font=("Arial", 13, "bold"), command=self.run)
        self.btn_run.pack(side="left", padx=10)
        self.btn_stop = ctk.CTkButton(btn_frame, text="STOP", fg_color="#e74c3c", width=100, height=40, state="disabled", command=self.stop_process)
    ...
    def toggle_inputs(self, state):
        ...
        for w in [self.blur_slider, self.blur_entry, self.shift_slider, self.shift_entry, self.preset_menu, self.batch_entry]:
    ...
    def update_scope_vals(self, b, s, cur, total):
        ...
        # FIX: Strict rounding to one decimal
        s_rounded = round(float(s), 1)
        b_pass, s_pass = b >= b_target, s <= s_target
        
        self.blur_scope_label.configure(
            text=f"{'✅' if b_pass else '❌'} SHARPNESS: {int(b)} / {int(b_target)}",
            text_color="#2ecc71" if b_pass else "#e74c3c"
        )
        self.shift_scope_label.configure(
            text=f"{'✅' if s_pass else '❌'} STABILITY: {s_rounded:.1f} / {s_target:.1f}",
            text_color="#2ecc71" if s_pass else "#e74c3c"
        )
        self.blur_meter.set(min(b / 250, 1.0))
        self.shift_meter.set(min(s / 20, 1.0))
    ...
    def run_logic(self):
        self.proc = SLAMProcessor(self.path_var.get(), self.batch_entry.get(), float(self.blur_entry.get()), 
                                 float(self.shift_entry.get()), lambda m: self.log_box.insert("end", f"> {m}\n"), 
    ...
    def on_finish(self):
        ...
        self.toggle_inputs(True); messagebox.showinfo("Complete", "Quality Check Complete. See CSV for 'Unclear' tags.")
